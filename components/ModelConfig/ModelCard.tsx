...
const ModelCard: React.FC<ModelCardProps> = ({
  ...
  const renderChatParams = (params: ChatModelParams) => (
    <div className="grid grid-cols-2 gap-4">
      <div>
        <label className="text-[10px] text-zinc-500 block mb-1">{t('params.temperatureLabel')}</label>
        <input
          type="number"
          min="0"
          max="2"
      ...
      <div>
        <label className="text-[10px] text-zinc-500 block mb-1">{t('params.maxTokensLabel')}</label>
        <input
          type="number"
          min="1"
          max="128000"
          ...
          placeholder={t('params.maxTokensPlaceholder')}
          className="w-full bg-white/[0.06] border border-white/10 rounded-xl px-3 py-2 text-xs text-white"
        />
        <p className="text-[9px] text-zinc-600 mt-1">{t('params.maxTokensHelper')}</p>
      </div>
    </div>
  );
  ...
  const renderImageParams = (params: ImageModelParams) => (
    <div>
      <label className="text-[10px] text-zinc-500 block mb-1">{t('params.defaultRatioLabel')}</label>
      <div className="flex gap-2">
        ...
        {(params.supportedAspectRatios || ['16:9', '9:16']).map((ratio) => (
          ...
          >
            {ratio === '16:9' ? t('common:aspectRatio.landscape') : ratio === '9:16' ? t('common:aspectRatio.portrait') : t('common:aspectRatio.square')}
          </button>
        ))}
      </div>
    </div>
  ...
  const renderVideoParams = (params: VideoModelParams) => (
    <div className="space-y-4">
      ...
      {editParams.supportedDurations.length > 1 && (
        <div>
          <label className="text-[10px] text-zinc-500 block mb-1">{t('params.defaultDurationLabel')}</label>
          <div className="flex gap-2">
            {editParams.supportedDurations.map((duration: VideoDuration) => (
              ...
              >
                {t('common:aspectRatio.durationValue', { seconds: duration })}
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="text-[10px] text-zinc-600">
        {t('videoMode.summaryPrefix')}
        {editParams.mode === 'sync'
          ? t('videoMode.summarySync')
          : editParams.mode === 'async'
          ? t('videoMode.summaryAsync')
          : t('videoMode.doubao')}
      </div>
    </div>
  );
      ...
      <div className="p-4 flex items-center justify-between">
        <div className="flex items-center gap-3 flex-1">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2">
              ...
              {model.isBuiltIn && (
                <span className="px-1.5 py-0.5 bg-zinc-700 text-zinc-400 text-[9px] rounded">{t('badges.builtIn')}</span>
              )}
            </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            title={model.isEnabled ? t('card.disableTooltip') : t('card.enableTooltip')}
          >
            {model.isEnabled ? (
          ...
          <div className="pt-4 space-y-4">
            ...
            <div>
              <label className="text-[10px] text-zinc-500 block mb-1">
                {t('card.apiKeyCardLabel')}
              </label>
              <input
                type="password"
                value={editApiKey}
                onChange={(e) => handleApiKeyChange(e.target.value)}
                placeholder={t('apiKey.placeholderUseGlobal')}
              ...
              />
              {model.apiKey && (
                <p className="text-[9px] text-green-500 mt-1">{t('card.apiKeyConfiguredBadge')}</p>
              )}
            </div>
