...
const AddModelForm: React.FC<AddModelFormProps> = ({ type, onSave, onCancel }) => {
  ...
  const handleSave = () => {
    if (!name.trim() || !apiModel.trim()) {
      showAlert(t('addModel.validationRequired'), { type: 'warning' });
      return;
    }
  ...
  return (
    <div className="bg-white/[0.045] border border-white/10 rounded-2xl p-4 space-y-4">
      <h4 className="text-sm font-bold text-white">{t('addModel.title')}</h4>

      {/* 基础信息 */}
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="text-[10px] text-zinc-500 block mb-1">{t('addModel.nameLabel')}</label>
          <input
            type="text"
            value={name}
        ...
        <div>
          <label className="text-[10px] text-zinc-500 block mb-1">{t('addModel.apiModelLabel')}</label>
          <input
            type="text"
            value={apiModel}
            onChange={(e) => setApiModel(e.target.value)}
            placeholder={t('addModel.apiModelPlaceholder')}
          ...
          <p className="text-[9px] text-zinc-600 mt-1">
            {t('addModel.apiModelHelper')}
          </p>
        </div>
      </div>

      <div>
        <label className="text-[10px] text-zinc-500 block mb-1">{t('addModel.descriptionLabel')}</label>
        <input
          type="text"
          value={description}
      ...
      {/* API 端点 */}
      <div>
        <label className="text-[10px] text-zinc-500 block mb-1">{t('addModel.endpointLabel')}</label>
        <input
          type="text"
          value={endpoint}
        ...
        />
        <p className="text-[9px] text-zinc-600 mt-1">
          {t('addModel.endpointHelper')}
        </p>
      </div>

      {/* 模型专属 API Key（可选） */}
      <div>
        <label className="text-[10px] text-zinc-500 block mb-1">{t('addModel.apiKeyLabel')}</label>
        <input
          type="password"
          value={apiKey}
      ...
      {type === 'video' && (
        <div>
          <label className="text-[10px] text-zinc-500 block mb-1">{t('videoMode.label')}</label>
