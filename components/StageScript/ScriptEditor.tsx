    ...
    <div className="flex-1 flex flex-col bg-slate-950/25 relative">
      <div className="h-16 border-b border-white/10 flex items-center justify-between px-8 bg-slate-950/45 backdrop-blur-xl shrink-0">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs font-bold text-cyan-100/75">{t('scriptEditor.heading')}</span>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={onContinue}
          ...
          >
            {isContinuing ? (
              <>
                ...
                {t('scriptEditor.continueButton.loading')}
              </>
            ) : (
              <>
                <Plus className="w-3.5 h-3.5" />
                {t('scriptEditor.continueButton.idle')}
              </>
            )}
          </button>
          <button
          ...
          >
            {isRewriting ? (
              <>
                <BrainCircuit className="w-3.5 h-3.5 animate-spin" />
                {t('scriptEditor.rewriteButton.loading')}
              </>
            ) : (
              <>
                <RotateCw className="w-3.5 h-3.5" />
                {t('scriptEditor.rewriteButton.idle')}
              </>
            )}
          </button>
      ...
      <div className="h-8 border-t border-white/10 bg-slate-950/45 px-4 flex items-center justify-end gap-4 text-[10px] text-slate-500 font-mono select-none">
        <span>{t('scriptEditor.statsCharacters', { count: stats.characters })}</span>
        <span>{t('scriptEditor.statsLines', { count: stats.lines })}</span>
        <div className="flex items-center gap-1.5">
          <div className="w-1.5 h-1.5 rounded-full bg-cyan-300/40"></div>
          {lastModified ? t('scriptEditor.savedStatus') : t('scriptEditor.readyStatus')}
        </div>
      </div>
    </div>
  );
