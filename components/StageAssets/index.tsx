...
const StageAssets: React.FC<Props> = ({ project, updateProject, onApiKeyError }) => {
  ...
  const handleBatchGenerate = async (type: 'character' | 'scene') => {
    ...
    if (isRegenerate) {
      showAlert(t(type === 'character' ? 'alerts.regenerateAllCharactersConfirm' : 'alerts.regenerateAllScenesConfirm'), {
        type: 'warning',
        showCancel: true,
        onConfirm: async () => {
  ...
  const handleAddCharacterToLibrary = (char: Character) => {
    const saveItem = async () => {
      try {
        ...
        showAlert(t('alerts.addedToLibrary', { name: char.name }), { type: 'success' });
        refreshLibrary();
      } catch (e: any) {
        showAlert(e?.message || t('alerts.addToLibraryFailed'), { type: 'error' });
      }
    };

    if (!char.referenceImage) {
      showAlert(t('alerts.addCharacterNoImageConfirm'), {
        type: 'warning',
        showCancel: true,
        onConfirm: saveItem
      });
  ...
  const handleAddSceneToLibrary = (scene: Scene) => {
    const saveItem = async () => {
      try {
        ...
        await saveAssetToLibrary(item);
        showAlert(t('alerts.addedToLibrary', { name: scene.location }), { type: 'success' });
        refreshLibrary();
      } catch (e: any) {
        showAlert(e?.message || t('alerts.addToLibraryFailed'), { type: 'error' });
      }
    ...
    if (!scene.referenceImage) {
      showAlert(t('alerts.addSceneNoImageConfirm'), {
        type: 'warning',
        showCancel: true,
        onConfirm: saveItem
      });
  ...
  const handleImportFromLibrary = (item: AssetLibraryItem) => {
    try {
      ...
      showAlert(t('alerts.imported', { name: item.name }), { type: 'success' });
    } catch (e: any) {
      showAlert(e?.message || t('alerts.importFailed'), { type: 'error' });
    }
  };

  const handleReplaceCharacterFromLibrary = (item: AssetLibraryItem, targetId: string) => {
    if (item.type !== 'character') {
      showAlert(t('alerts.selectCharacterAssetWarning'), { type: 'warning' });
      return;
    }
    if (!project.scriptData) return;
    ...
    showAlert(t('alerts.characterReplaced', { from: previous.name, to: cloned.name }), { type: 'success' });
  ...
  const handleDeleteLibraryItem = async (itemId: string) => {
    try {
      ...
      showAlert(e?.message || t('alerts.deleteAssetFailed'), { type: 'error' });
    }
  };
  ...
  const handleAddCharacter = () => {
    ...
    updateProject({ scriptData: newData });
    showAlert(t('alerts.characterCreated'), { type: 'success' });
  };
  ...
  const handleDeleteCharacter = (charId: string) => {
    ...
    showAlert(
      t('alerts.deleteCharacterConfirm.message', { name: char.name }),
      {
        type: 'warning',
        title: t('alerts.deleteCharacterConfirm.title'),
        showCancel: true,
        ...
        onConfirm: () => {
          ...
          updateProject({ scriptData: newData });
          showAlert(t('alerts.characterDeleted', { name: char.name }), { type: 'success' });
        }
      }
    );
  };
  ...
  const handleAddScene = () => {
    ...
    updateProject({ scriptData: newData });
    showAlert(t('alerts.sceneCreated'), { type: 'success' });
  };
  ...
  const handleDeleteScene = (sceneId: string) => {
    ...
    showAlert(
      t('alerts.deleteSceneConfirm.message', { name: scene.location }),
      {
        type: 'warning',
        title: t('alerts.deleteSceneConfirm.title'),
        showCancel: true,
        confirmText: t('common:actions.delete'),
        cancelText: t('common:actions.cancel'),
        onConfirm: () => {
          ...
          updateProject({ scriptData: newData });
          showAlert(t('alerts.sceneDeleted', { name: scene.location }), { type: 'success' });
        }
      }
    );
  };
  ...
  const handleGenerateVariation = async (charId: string, varId: string) => {
    ...
    try {
      ...
      showAlert(t('alerts.variationGenerationFailed'), { type: 'error' });
    }
  };
    ...
    <div className={STYLES.mainContainer}>
      ...
      {batchProgress && (
        <div className="absolute inset-0 z-50 bg-black/80 flex flex-col items-center justify-center backdrop-blur-md animate-in fade-in">
          ...
          <h3 className="text-xl font-bold text-white mb-2">{t('batchProgress.heading')}</h3>
          ...
          <p className="text-zinc-400 font-mono text-xs">
            {t('batchProgress.progressLabel', { current: batchProgress.current, total: batchProgress.total })}
          </p>
        </div>
      )}
          ...
          <div className={STYLES.modalContainer} onClick={(e) => e.stopPropagation()}>
            <div className={STYLES.modalHeader}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-sm font-bold text-white">{t('assetLibrary.title')}</div>
              ...
              <div className="flex flex-wrap items-center gap-3 mb-6">
                ...
                <div className="flex gap-2">
                  {(['all', 'character', 'scene'] as const).map((type) => (
                    ...
                    >
                      {type === 'all' ? t('assetLibrary.filterAll') : type === 'character' ? t('assetType.character') : t('assetType.scene')}
                    </button>
                  ))}
                </div>
              </div>
                        ...
                        <div className="p-4 space-y-3">
                          <div>
                            ...
                            <div className="text-[10px] text-zinc-500 font-mono uppercase tracking-widest mt-1">
                              {item.type === 'character' ? t('assetType.character') : t('assetType.scene')}
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                            ...
                            >
                              {replaceTargetCharId ? t('assetLibrary.replaceButton') : t('assetLibrary.importButton')}
                            </button>
                            <button
                              onClick={() =>
                                showAlert(t('assetLibrary.confirmDelete'), {
                                  type: 'warning',
                                  showCancel: true,
      ...
      <div className={STYLES.header}>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-zinc-500 uppercase">{t('header.modelLabel')}</span>
            <ModelSelector
              type="image"
          ...
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-zinc-500 uppercase">{t('header.ratioLabel')}</span>
            <AspectRatioSelector
              value={aspectRatio}
          ...
          <div className="flex items-end justify-between mb-6 border-b border-white/10 pb-4">
            <div>
              <h3 className="text-sm font-bold text-white uppercase tracking-widest flex items-center gap-2">
                ...
                {t('casting.sectionTitle')}
              </h3>
              <p className="text-xs text-zinc-500 mt-1 pl-3.5">{t('casting.sectionDesc')}</p>
            </div>
            <div className="flex gap-2">
              <button
              ...
              >
                ...
                {allCharactersReady ? t('casting.regenerateAllButton') : t('casting.generateAllButton')}
              </button>
            </div>
          </div>
          ...
          <div className="flex items-end justify-between mb-6 border-b border-white/10 pb-4">
            <div>
              <h3 className="text-sm font-bold text-white uppercase tracking-widest flex items-center gap-2">
                ...
                {t('scenes.sectionTitle')}
              </h3>
              <p className="text-xs text-zinc-500 mt-1 pl-3.5">{t('scenes.sectionDesc')}</p>
            </div>
            <div className="flex gap-2">
              <button
              ...
              >
                ...
                {allScenesReady ? t('scenes.regenerateAllButton') : t('scenes.generateAllButton')}
              </button>
            </div>
          </div>
